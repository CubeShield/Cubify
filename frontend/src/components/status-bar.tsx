import { TerminalSquare, ChevronUp } from 'lucide-react'
import { useLogs } from '../context/log-context'

export function StatusBar() {
	const { lastLog, setIsOpen, isOpen } = useLogs()

	return (
		<div
			onClick={() => setIsOpen(!isOpen)}
			className='fixed bottom-0 left-0 right-0 h-8 bg-zinc-950 text-white text-xs flex items-center px-4 border-t border-white/10 cursor-pointer hover:bg-zinc-900 transition-colors z-40'
		>
			<TerminalSquare className='w-3 h-3 mr-2 text-primary' />
			<span className='opacity-80 truncate flex-1 font-mono'>{lastLog}</span>
			<ChevronUp
				className={`w-3 h-3 opacity-50 transition-transform ${isOpen ? 'rotate-180' : ''}`}
			/>
		</div>
	)
}
